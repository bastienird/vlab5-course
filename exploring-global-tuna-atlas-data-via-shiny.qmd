...
1. From the **Shiny Apps tab** in VLab → directly launch stable or dev versions of available apps.
2. From the repository in `~/GitHubRepos` → open the `.Rproj`, restore packages, and run `shiny::runApp()`.
...
If you want to run the shiny apps but are not yet a member of the Vlab 5, you can use the following command in your terminal (if you already have docker installed). Warning the docker images can be heavy and need certain amount of RAM for running.

```bash
docker pull ghcr.io/<name-of-the-shiny-app-repo-eg.firms-gta/tunaatlas_pie_map_shiny>
docker run ghcr.io/<name-of-the-shiny-app-repo-eg.firms-gta/tunaatlas_pie_map_shiny>
```
...
4. **Visualize** → interactive map (e.g., 5° grid) + plots; export as needed (see README).
...
4. **Inspect differences** → tables/plots of differences, coverage, and changes.  
